...
namespace devexpress.View
{
    public partial class DanhSachPhong : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            gvDataList.OptionsNavigation.AutoFocusNewRow = true;
            gvDataList.FocusedRowHandle = gvDataList.RowCount;
            t = 1;
            btnThem.Enabled = false;
        ...
        {
            gvDataList.SetRowCellValue(gvDataList.FocusedRowHandle, gvDataList.Columns[2], 1);
        ...
        private void gvDataList_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            else if (view.FocusedColumn == grcSoGiuong)
            {
                if (Convert.ToInt32(e.Value) <= 0)
                {
                    e.Valid = false;
            ...
            else if (view.FocusedColumn == grcSoNguoi)
            {
                if (Convert.ToInt32(e.Value) <= 0 )
                {
                    e.Valid = false;
            ...
            else if (view.FocusedColumn == grcPhanLoai)
            {
                if (Convert.ToInt32(e.Value) <= 0)
                {
                    e.Valid = false;
